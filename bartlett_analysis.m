...
%eta=vertcat(eye(N), zeros(1,N));
eta=vertcat(chol_my, zeros(1,N));
% eta=vertcat(sig_chol_ee, zeros(1,N));
...
s_y_b_s=zeros(A,N,SIM);
s_y_b20_agg_s=zeros(A,SIM);
s_y_b15_agg_s=zeros(A,SIM);
s_y_b10_agg_s=zeros(A,SIM);
s_y_b5_agg_s=zeros(A,SIM);
...
% YY =kron(Y',ones(1,T)');
% yys=(yys-yysl).*(YY./yysl);
% yys=yys(3:T,:);

[T1,C]=size(yys);

yhat = mean(yys); % mean of columns of matrix X
yhat =kron(yhat,ones(1,T1)');
Yc0=yys-yhat; % centered data matrix
Sigma0 = (1/T1)*(Yc0'*Yc0); % cov(yys) gets the same except that divide by T1-1! SAME THINGS!
...
for j=1:A
  ...
  s_y_b5(:,:,j)=s_y_b5(:,:,j)+ (1-k/(q5+1))*(sigmas(:,:,k)*exp(-1i*k*w(j))+sigmas(:,:,k)'*exp(1i*k*w(j)));
  end
    for k=1:q10
    if k==1
    s_y_b10(:,:,j)=Sigma0;
    end
  s_y_b10(:,:,j)=s_y_b10(:,:,j)+ (1-k/(q10+1))*(sigmas(:,:,k)*exp(-1i*k*w(j))+sigmas(:,:,k)'*exp(1i*k*w(j)));
    end
    for k=1:q15
    if k==1
    s_y_b15(:,:,j)=Sigma0;
    end
  s_y_b15(:,:,j)=s_y_b15(:,:,j)+ (1-k/(q15+1))*(sigmas(:,:,k)*exp(-1i*k*w(j))+sigmas(:,:,k)'*exp(1i*k*w(j)));
    end
    for k=1:q20
    if k==1
    s_y_b20(:,:,j)=Sigma0;
    end
  s_y_b20(:,:,j)=s_y_b20(:,:,j)+ (1-k/(q20+1))*(sigmas(:,:,k)*exp(-1i*k*w(j))+sigmas(:,:,k)'*exp(1i*k*w(j)));
    end
end

varcov20=sum(s_y_b20,3);
varcov15=sum(s_y_b15,3);
varcov10=sum(s_y_b10,3);
varcov5=sum(s_y_b5,3);

s_y_b20_agg=zeros(A,1);
s_y_b15_agg=zeros(A,1);
s_y_b10_agg=zeros(A,1);
s_y_b5_agg=zeros(A,1);

%varcov15=ones(N,N);varcov10=ones(N,N);varcov5=ones(N,N);varcov3=ones(N,N);

for j=1:A
s_y_b20_agg(j)=shry'*(s_y_b20(:,:,j)./varcov20)*shry;
s_y_b15_agg(j)=shry'*(s_y_b15(:,:,j)./varcov15)*shry;
s_y_b10_agg(j)=shry'*(s_y_b10(:,:,j)./varcov10)*shry;
s_y_b5_agg(j)=shry'*(s_y_b5(:,:,j)./varcov5)*shry;
end

s_y_b20_agg_s(:,i)=s_y_b20_agg;
s_y_b15_agg_s(:,i)=s_y_b15_agg;
s_y_b10_agg_s(:,i)=s_y_b10_agg;
s_y_b5_agg_s(:,i)=s_y_b5_agg;

end

s_y_agg20=mean(s_y_b20_agg_s,2);
s_y_agg15=mean(s_y_b15_agg_s,2);
s_y_agg10=mean(s_y_b10_agg_s,2);
s_y_agg5=mean(s_y_b5_agg_s,2);

toc

    if RHO==-1
    h=figure;
    plot(w,[s_y_agg s_y_agg5 s_y_agg10 s_y_agg15 s_y_agg20],'LineWidth', 2);
    legend('Actual', ' Bartlett q=5', ' Bartlett q=10', ' Bartlett q=15',' Bartlett q=70', 'location','northeast')
    title('Intratemporal Adjustment Cost Model Spectrum Aggregate Output Growth RHO=-1')
    saveas(h,'E:\Networks_in_Macroeconomics\Tex\bartlett_our_-1.png');
    ...
    h=figure;
    plot(w,[s_y_agg s_y_agg5 s_y_agg10 s_y_agg15 s_y_agg20],'LineWidth', 2);
    legend('Actual', ' Bartlett q=5', ' Bartlett q=10', 'Bartlett q=15',' Bartlett q=30','location','northeast')
    title('Intratemporal Adjustment Cost Model Spectrum Aggregate Output Growth RHO=-2')
    saveas(h,'E:\Networks_in_Macroeconomics\Tex\bartlett_our_-2.png');
