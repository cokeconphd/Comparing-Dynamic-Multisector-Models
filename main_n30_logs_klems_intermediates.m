...
VAR=1;% 1 to estimate VAR(1) data and models and then IRFs
...
data0=csvread('data_labor_klems.csv');
[Tip,C]=size(data0);
...
yysl=lagmatrix(ll,1);
yys=ll-yysl;  %they are in log deviation so just y-yl es percent change 
yys=yys(2:T,:);

yyscl=lagmatrix(llc,1);
yysc=llc-yyscl;
yysc=yysc(2:T,:);

yyspl=lagmatrix(llp,1);
yysp=llp-yyspl;
yysp=yysp(2:T,:);
...
title('All Simulated Kernel Implied Shocks')
title('Sectoral Labor Pairwise Correlation Kernel Density')
...
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\sim_kernel_labor_klems_-2.png');
...
legend('Data Growth','Carvalho','Foerster et al', 'One K RHO=-1', 'One K Factor', 'Foerster et al Factor')
%saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\sim_kernel_klems_-1.png');
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\sim_kernel_labor_klems_-1.png');
...
corr_y_k_my=zeros(SIM,1);
corr_y_k_c=zeros(SIM,1);
corr_y_k_p=zeros(SIM,1);
...
[YY,XX,e]=simu_1st_original(gx, hx, eta, T, x0, e0);
yys=YY(:,(2*N+1:3*N));
kk=YY(:,(N+1:2*N)); %current sectoral capital 
cc=YY(1:T,1:N);
aa=XX(1:T,2:N+1);
ll0=YY(:,(3*N+1:4*N));
kk1=kk(2:T,:); %tomorrow's sectoral capital
ll=kk1;
...
ll_a=sum(ll,2); %tomorrow's level of aggregate capital without considering RHO
%ll_a_adjcost=(sum(ll.^(-RHO),2)).^(-1/RHO); %tomorrow's level of aggregate capital with adj costs

yysl_y=lagmatrix(yys,1);
yysl_y=yys-yysl_y;
yysl_ya=yysl_y*shry;
yysl_ya=yysl_ya(2:T,:);

yysl=lagmatrix(ll,1); %growth rate of sectoral capital
yys=ll-yysl;
yys=yys(2:T-1,:);

yysl_a=lagmatrix(ll_a,1); %growth rate of aggregate capital
yys_a=ll_a-yysl_a;
yys_a=yys_a(2:T-1,:);
...
yyscl_ya=lagmatrix(yysc_a0,1);
yysc_ya=yysc_a0-yyscl_ya;
yysc_ya=yysc_ya(2:T,:);
...
yyspl_ya=lagmatrix(yysp_a0,1);
yysp_ya=yysp_a0-yyspl_ya;
yysp_ya=yysp_ya(2:T,:);
...
yyscl_a=lagmatrix(llc_a,1);
yysc_a=llc_a-yyscl_a;
yysc_a=yysc_a(2:T,:);
...
yyspl_a=lagmatrix(llp_a,1);
yysp_a=llp_a-yyspl_a;
yysp_a=yysp_a(2:T,:);

%corr_y_k_c(i)=corr(yysc_a,yysc_ya);
%corr_y_k_p(i)=corr(yysp_a,yysp_ya);
%corr_y_k_my(i)=corr(yys_a,yysl_ya);

%std_y_my(i)=std(yys_ya);
%std_k_my(i)=std(yys_a);
%std_y_c(i)=std(yysc_ya);
%std_k_c(i)=std(yysc_a);
%std_y_p(i)=std(yysp_ya);
%std_k_p(i)=std(yysp_a);
...
datal0=csvread('data_labor_klems.csv');

dy_la0=(datal0-lagmatrix(datal0,1))./lagmatrix(datal0,1);
dy_la0=dy_la0(2:Tip,:);
dy_la0=detrend(dy_la0,'constant');

dy_la0=dy_la0*shry;

yysld_a=sum(datal0,2);
dy_la=(yysld_a-lagmatrix(yysld_a,1))./lagmatrix(yysld_a,1);
dy_la=dy_la(2:Tip,:);
dy_la=detrend(dy_la,'constant');
corr(dy_la, dy_la0)

datay0=csvread('data_output_klems.csv');

dy_ya0=(datay0-lagmatrix(datay0,1))./lagmatrix(datay0,1);
dy_ya0=dy_ya0(2:Tip,:);
dy_ya0=detrend(dy_ya0,'constant');

dy_ya0=dy_ya0*shry;

[Tip,C]=size(datay0);
yysyd_a=sum(datay0,2);
dy_ya=(yysyd_a-lagmatrix(yysyd_a,1))./lagmatrix(yysyd_a,1);
dy_ya=dy_ya(2:Tip,:);
dy_ya=detrend(dy_ya,'constant');

data0=csvread('data_capital_klems.csv');

dy_a0=(data0-lagmatrix(data0,1))./lagmatrix(data0,1);
dy_a0=dy_a0(2:Tip,:);
dy_a0=detrend(dy_a0,'constant');
...
dy_a2=dy_a2(2:Tip,:);
dy_a=detrend(dy_a2,'constant');

corr(dy_a, dy_a0)

disp('std(Y)  std(K)  (data, c, fsw, our)')
disp('double check this. Using indices is right for levels, is adding up right for comovement?')
disp([std(dy_ya) std(dy_a) ]);
%disp([mean(std_y_c) mean(std_k_c) ]);
%disp([mean(std_y_p) mean(std_k_p) ]);
%disp([mean(std_y_my) mean(std_k_my) ]);

disp('Corr(K,Y) data  c  fsw my ')
%disp([corr(dy_a,dy_ya) mean(corr_y_k_c) mean(corr_y_k_p) mean(corr_y_k_my)]);
