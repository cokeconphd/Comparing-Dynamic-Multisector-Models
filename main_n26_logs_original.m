%Miranda-Pinto and Young 2017: Solves a N sector 1 capital model DSGE model
%with sectoral linkages, calibrated for the U.S. industrial sectors
...
CORRELATIONS=1; %1 to calculate implied distribution of pairwise correlation across sectors
...
load gamma26 %I-O matrix
load alfa26 %capital shares
load av_sell_theta26 %average capital flow matrix

av_theta_row = av_sell_theta26;
[N, col]=size(GAMA);
...
shry = xlsread('1997DetailedItemOutput.xlsx',4,'f3:f28' ); %observed sectoral shares 1997 IO
T=500; %size of the simulated sample from models
SIM=500; %number of simulation for each VAR estimation
...
NK=19; %Capital good sector for case when sector nk is the only capital producer
...
if RHO==-1  
save ('eps_my_-1.mat' ,'eps_my') %saving backed out erros in case want to compare them with other models
else
...
% eta=vertcat( zeros(1,N),eye(N)*0.02);  % average diagonal of chol_c
% eta_c=vertcat( zeros(N,N), eye(N)*0.02); 
% eta_p=vertcat(zeros(N,N), eye(N)*0.02); 

%Pairwise correlation sectoral shocks

corr_eps_p=corr(eps_p);
mask = tril(true(size(corr_eps_p)),-1);
pierres = corr_eps_p(mask);
mean(pierres)

corr_eps=corr(eps_my);
mask2 = tril(true(size(corr_eps)),-1);
ours = corr_eps(mask2);
mean(ours)

corr_eps_c=corr(eps_c);
mask3 = tril(true(size(corr_eps_c)),-1);
carv = corr_eps_c(mask3);
mean(carv)
...
%normality test
%aver=jbtest(eps_my(:,21))

if RHO<-1

h=figure
plot(xid',[fd' fcs fps fmys], 'LineWidth' , 2);
title('Pairwise Correlations Sectoral Output Growth (\rho<-1)')
legend('Data Growth','Carvalho','Foerster et al', 'One K \rho<-1' )
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\sim_kernel_-2.png');
    
h=figure;
plot(sector',cor_dy_d(:,19),'LineWidth' , 2);
legend('Data')
hold on
plot(sector',cor_dy_c(:,19),'LineWidth' , 2);
legend('Carvalho')
hold on
plot(sector',cor_dy_p(:,19),'LineWidth' , 2);
legend('Foerster et al 2011')
hold on
plot(sector',cor_dy_my(:,19),'LineWidth' , 2);
legend('One K \rho<-1')
hold off
title('Pairwise correlations with Machinery')
xlabel('Sectors')
ylabel('Correlation with Machinery')
legend('Data','Carvalho','Foerster et al', 'One K \rho<-1')
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\corr_machinery_-2.png');
...
[fcons1,xi]=ksdensity(eps_my(:,19));
[fconsp,xi0]=ksdensity(eps_p(:,19),xi);

h=figure
plot(xi,[fconsp' fcons1'], 'LineWidth' , 2);
title('Kernel shocks to Machinery')
%title('Sectoral Output Pairwise Correlation Kernel Density')
legend('FSW','One K $\rho=-1.1$')
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\kernel_IPshock4_-2.png');
...
title('Pairwise Correlations Sectoral Output Growth (\rho=-1)')
legend('Data Growth','Carvalho','Foerster et al', 'One K \rho=-1' )
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\sim_kernel_-1.png');
...
ylabel('Correlation with Machinery')
legend('Data','Carvalho','Foerster et al', 'One K \rho=-1')
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\corr_machinery_-1.png');
...
xlabel('Sectors 1 to 26','LineWidth', 2) % x-axis label
ylabel('Correlation errors different models','LineWidth', 2) % y-axis label
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\corr_err_p_-1.png');
...
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\kernel_IPshock4_-1.png');
...
% 1) Stationary Sectoral Shocks: Barlett Estimate

%Carvalho 2007

[A,B]=size(w');

M_carvalho=xlsread('m_pii_26_carvalho_099.xlsx',1,'a1:az52');
PII_carvalho=xlsread('m_pii_26_carvalho_099.xlsx',2,'a1:az78');
PIIA_carvalho=xlsread('m_pii_26_carvalho_099.xlsx',3,'a1:z26');
M_carvalho(N+1:2*N,N+1:2*N)= PII_carvalho(2*N+1:3*N,N+1:2*N);

% %Pierre's model
M_pierre=xlsread('m_pii_26_pierre_099.xlsx',1,'a1:az52');
PII_pierre=xlsread('m_pii_26_pierre_099.xlsx',2,'a1:az78');
PIIA_pierre=xlsread('m_pii_26_pierre_099.xlsx',3,'a1:z26');
M_pierre(N+1:2*N,N+1:2*N)= PII_pierre(2*N+1:3*N,N+1:2*N);

s_y_s=zeros(A,N,SIM);
s_y_agg_s=zeros(A,SIM);

s_y_c_s=zeros(A,N,SIM);
s_y_agg_c_s=zeros(A,SIM);

s_y_p_s=zeros(A,N,SIM);
s_y_agg_p_s=zeros(A,SIM);

s_y_sec=zeros(A,N);
s_y_c_sec=zeros(A,N);
s_y_p_sec=zeros(A,N);
s_y_ip_sec=zeros(A,N);
...
[YYc,XXc,e]=simu_1st_original(PII_carvalho(1:N,1:2*N),M_carvalho, eta_c, T, x0p, e0); %SHOULD I GIVE SD NOT VAR?
cc=YYc;
kk=XXc(1:T,1:N);
aa=XXc(1:T,N+1:2*N);
yysc=zeros(T,N);

%Recover Y using equation (33) in Foerster et al 2011 online appendix
...
%Bartlett one capital model

yhat = mean(yys); 
yhat =kron(yhat,ones(1,T1)');
Yc0=yys-yhat; % centered data matrix
Sigma0 = (1/T1)*(Yc0'*Yc0); 

%Bandwidth Bartlett estimate
...
for j=1:A
  ...
  s_y(:,:,j)=s_y(:,:,j)+ (1-k/(q+1))*(sigmas(:,:,k)*exp(-1i*k*w(j))+sigmas(:,:,k)'*exp(1i*k*w(j)));
  end
end

s_y=s_y./(2*pi);

%Variance covariance matrix to normalize spectral density
...
%Aggregate spectral density using observed output shares
...
for j=1:A
    s_y_sec(j,:)=diag(s_y(:,:,j))'./diag(varcov)';
end

% Bartlett Carvalho 07
...
for j=1:A
    s_y_c_sec(j,:)=diag(s_y_c(:,:,j))'./(diag(varcov)');
end
...
for j=1:A
    s_y_p_sec(j,:)=diag(s_y_p(:,:,j))'./(diag(varcov)');
end
...
s_y2=mean(s_y_s,3);
s_y_agg2=mean(s_y_agg_s,2);

s_y_c2=mean(s_y_c_s,3);
s_y_agg_c2=mean(s_y_agg_c_s,2);

s_y_p2=mean(s_y_p_s,3);
s_y_agg_p2=mean(s_y_agg_p_s,2);
...
yhat = mean(yysd); 
yhat =kron(yhat,ones(1,T1)');
Yc0=yysd-yhat; % centered data matrix
Sigma0 = (1/T1)*(Yc0'*Yc0); % covariance matrix
...
for j=1:A
  ...
  s_y_ip(:,:,j)=s_y_ip(:,:,j)+ (1-k/(q+1))*(sigmas(:,:,k)*exp(-1i*k*w(j))+sigmas(:,:,k)'*exp(1i*k*w(j)));
  end
end
...
for i=1:A
    s_y_ip_sec(i,:)=diag(s_y_ip(:,:,i))'./(diag(varcov)');
end

s_y_agg_ip=zeros(A,1);

for j=1:A
s_y_agg_ip(j)=shry'*(s_y_ip(:,:,j)./varcov)*shry;
end

if RHO<-1
h=figure;
plot(w,[s_y_agg_c2  s_y_agg_p2 s_y_agg2 s_y_agg_ip],'LineWidth', 2);
legend('Carvalho', 'Foerster et al', '\rho=-1.2','Data','location','northeast')
title('Spectral Density Aggregate Ouput Growth')
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\spectral_-2,1_chol.png')

h=figure;
plot(w,[s_y_c2(:,19) s_y_p2(:,19)  s_y2(:,19)  s_y_ip_sec(:,19)],'LineWidth', 2);
legend('Carvalho', 'Foerster et al', '\rho=-1.2','Data','location','northeast')
title('Spectral Density Ouput Growth Machinery')
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\spectral_19_-2,1_chol.png')

h=figure;
plot(w,[s_y_c2(:,4) s_y_p2(:,4)  s_y2(:,4)  s_y_ip_sec(:,4)],'LineWidth', 2);
legend('Carvalho', 'Foerster et al', '\rho=-1.2','Data','location','northeast')
title('Spectral Density Ouput Growth Food')
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\spectral_4-2,1_chol.png')
...
h=figure;
plot(w,[s_y_agg_c2  s_y_agg_p2 s_y_agg2 s_y_agg_ip],'LineWidth', 2);
legend('Carvalho', 'Foerster et al', 'One K \rho=-1','Data','location','northeast')
title('Spectral Density Aggregate Ouput Growth')
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\spectral_-1_chol.png')

h=figure;
plot(w,[s_y_c2(:,19) s_y_p2(:,19)  s_y2(:,19)  s_y_ip_sec(:,19)],'LineWidth', 2);
legend('Carvalho', 'Foerster et al', 'One K \rho=-1','Data','location','northeast')
title('Spectral Density Ouput Growth Machinery')
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\spectral_19_-1_chol.png')

h=figure;
plot(w,[s_y_c2(:,4) s_y_p2(:,4)  s_y2(:,4)  s_y_ip_sec(:,4)],'LineWidth', 2);
legend('Carvalho', 'Foerster et al', 'One K \rho=-1','Data','location','northeast')
title('Spectral Density Ouput Growth Food')
saveas(h,'C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\spectral_4-1_chol.png')
...
%aggip=xlsread('C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\code_to_eric\ipl2.xlsx',,'c2:ab145');
data0=xlsread('C:\Users\uqjmira1\Dropbox\Miranda-Young\Comparing_Models\code_to_eric\ipl2.xlsx',2,'c2:ab145');
...
%[IRF, Amat, Omega] = impulse(Betas,SIGMA,1,26,10);
